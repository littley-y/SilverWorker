...
`docs/PR_Review/`는 **단일 폴드**입니다. 역할별 서브폴드는 사용하지 않습니다.
...
1. **Implementer(OpenCode)**가 PR을 생성한다.
    ...
    - `docs/PR_Review/YYYY-MM-DD-[role]-pr<N>-request.md` 에 리뷰 요청 문서 작성 (Reviewer가 감지용)
    - `docs/PROGRESS.md` 해당 Spec 상태를 `🔄 리뷰 대기` 로 업데이트
2. Reviewer(Claude/Gemini)가 코드를 검수한다.
    - `gh pr list` 또는 `docs/PR_Review/`의 request.md 파일로 PR 감지
    - `gh pr view <PR#>` / `gh pr diff <PR#>`
    - `docs/planning/`의 명세서와 비교
3. Reviewer가 GitHub PR에 리뷰를 먼저 등록한다.
    - `gh pr review <PR#> --request-changes|--approve --body "리뷰 완료. 상세 피드백은 docs/PR_Review/...md 파일을 확인하세요."`
    - Implementer에게 GitHub 알림이 전달됨
4. Reviewer가 동일한 내용을 `docs/PR_Review/`에 마크다운 로그로 저장한다.
    - 파일명: `YYYY-MM-DD-[role]-pr<N>-review_[claude|gemini].md`
5. **Implementer(OpenCode)**는 GitHub 알림을 받고 `docs/PR_Review/`의 리뷰 파일을 읽어 내용을 반영하여 코드를 수정한다.
6. **승인(Approve)된 경우에만**: Reviewer가 `docs/PROGRESS.md` 해당 Spec 상태를 `✅ 완료` 로 최종 업데이트.
